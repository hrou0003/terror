...
impl PiecePool {
    pub(crate) fn new(
        ...
        let number_of_pieces = (torrent.info.length() as f64
            / torrent.info.piece_length.unwrap() as f64)
            .ceil() as usize;
        let block_size = 1 << 14;
            ...
            .map(|index| {
                ...
                let blocks = (0..number_of_blocks)
                    .map(|block_index| Block {
                        ...
                        block_size: if block_index == number_of_blocks - 1
                ...
                let files = match torrent.info.files.as_ref() {
                    ...
                    None => vec![FileInfo {
                        start_piece: Some(0),
                        end_piece: Some(torrent.get_number_of_pieces() - 1),
                        offset: Some(0),
                        priority: Some(Priority::default()),
                        length: torrent.info.length(),
                        path: vec![torrent.info.name.to_string()],
                        name: Some(torrent.info.name.to_string()),
                        md5sum: torrent.info.md5hash.clone(),
                    }],
                };
    ...
    pub async fn start(&mut self) {
        debug!("Starting Piece Pool");

        // Queue up the first 10 pieces
        self.queue_n_tasks(30).await;
        let mut downloaded_pieces: Vec<usize> = vec![];

        // Start listening on the response channel
        loop {
            debug!("am I listening?");
            let completed_task = self.completed_task_rx.recv().await;
            match completed_task {
                Some(CompletedTask::DownloadedBlock {
                    piece_index,
                    block_index,
                    bytes,
                    ..
                }) => {
                    debug!("Received block in piece pool");
                    let queue_size = self.task_tx.len();
                    debug!("Blocks left in queue: {}", queue_size);
                    if queue_size < 10 {
                        let number_of_queued_pieces = self.queue_n_tasks(10).await;
                        debug!("Queued 10 more blocks");
                    }
                    ...
                    {
                        let piece_bytes: Vec<u8> = piece
                            .blocks
                            .iter()
                            .flat_map(|block| match &block.block_state {
                                BlockState::Downloaded { data, .. } => data.clone(),
                                _ => Bytes::new(),
                            })
                            .collect();
                        piece.set_state(PieceState::Downloaded {
                            piece_bytes: piece_bytes.clone(),
                        });
                            ...
                            match piece.save_piece().await {
                                Ok(_) => {
                                    ...
                                    info!(
                                        ...
                                        (downloaded_pieces.len() as f32 / self.pieces.len() as f32)
                                            * 100 as f32
                                    );
                ...
                Some(CompletedTask::FailedBlock {
                    piece_index,
                    block_index,
                    ..
                }) => {
                    debug!(
                        "Failed to download block {} of piece {}",
                    ...
                    let piece = self
                        .pieces
                        .iter()
                        .find(|(&index, piece)| index == piece_index);
                    let block = piece
                        .unwrap()
                        .1
                        .blocks
    ...
    pub async fn queue_n_tasks(&mut self, n: usize) -> Option<usize> {
        ...
        debug!("Queued {} blocks with {} left in queue", n, task_tx.len());
        return Some(n);
    }
    ...
    fn get_next_piece(&self) -> Option<&Piece> {
        self.pieces
            .values()
            .find(|&piece| piece.piece_state == PieceState::Missing)
    }
    ...
    pub fn all_pieces_downloaded(&self) -> Option<bool> {
        Some(
            self.pieces
                .iter()
                .all(|(_, piece)| piece.piece_state == PieceState::Saved),
        )
    }
